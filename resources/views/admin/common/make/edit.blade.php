        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route($updateRoute, [$catIData->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('name') ? 'has-error' : '' }}">
                            <label class="required" for="name">{{ trans('global.name') }}</label>
                            <input class="form-control" type="text" name="name" id="name" value="{{ old('cat_name', $catIData->name) }}" required>
                        ...
                        <div class="form-group {{ $errors->has('item_types') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="item_types[]" id="item_types" multiple="multiple" required>
                                @foreach($itemTypes as $itemType)
                                    <option value="{{ $itemType->id }}" {{ in_array($itemType->id, old('item_types', $selectedItemTypes)) ? 'selected' : '' }}>
                                        {{ $itemType->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('description') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="description" id="description" value="{{ old('description', $catIData->description) }}">
                        ...
                        <div class="form-group {{ $errors->has('status') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control" name="status" id="status" required>
                                ...
                                @foreach(App\Models\VehicleMake::STATUS_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('status', $catIData->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
...
<script>
    ...
    init: function () {
@if(isset($catIData) && $catIData->image)
      var file = {!! json_encode($catIData->image) !!}
