        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} Location
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route($updateRoute, [$city->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('city_name') ? 'has-error' : '' }}">
                            <label class="required" for="city_name">{{ trans('global.location_name') }}</label>
                            <input class="form-control" type="text" name="city_name" id="city_name" value="{{ old('city_name', $city->city_name) }}" onchange="fetchLatLong()" required>
                      ...
                      <label class="required" for="country_code">{{ trans('global.country') }}</label>
                      <select id="country_code" name="country_code" class="form-control" onchange="fetchLatLong()" required>
                          @foreach($countries as $code => $name)
                              <option value="{{ $code }}" {{ old('country_code', $city->country_code) === $code ? 'selected' : '' }}>
                                  {{ $name }}
                              </option>
                          @endforeach
                      </select>
                      ...
                      @endif
                        ...
                        <div class="form-group {{ $errors->has('description') ? 'has-error' : '' }}">
                            ...
                            <textarea class="form-control ckeditor" name="description" id="description">{!! old('description', $city->description) !!}</textarea>
                        ...
                        <div class="form-group {{ $errors->has('latitude') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="latitude" id="latitude" value="{{ old('latitude', $city->latitude) }}" required>
                        ...
                        <div class="form-group {{ $errors->has('longtitude') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="longtitude" id="longtitude" value="{{ old('longtitude', $city->longtitude) }}" required>
